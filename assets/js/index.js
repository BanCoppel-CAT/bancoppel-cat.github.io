...
var gestionaComboCobranza = function(){
	...
	if($("#fin_gestion").val() !== "0")
	{
		$("#btnCobranza").removeClass("disabled");
	}
	else
	{
		$("#btnCobranza").addClass("disabled");
	}
...
var gestionaBotonCobranza = function(){
	if($("#btnCobranza").hasClass("disabled") == false)
	{
		if($("#fin_gestion").val() == "1")
		{
			if($("#importe_convenio").val() == "" || isNaN($("#importe_convenio").val()) == true)
			{
				M.toast({html: 'Por favor introduce un importe correcto.'});
			}
			else
			{
				if(parseInt($("#importe_convenio").val()) < 100 || parseInt($("#importe_convenio").val()) > 99999)
				{
					M.toast({html: 'Por favor introduce un importe entre 100 y 99999'});
				}
				else
				{
					if($("#fecha_convenio").val() == "" || $("#fecha_convenio").val() == null)
					{
						M.toast({html: 'Por favor introduce una fecha correcta.'});
					}
					else
					{
						$("#fin_gestion").attr("disabled", true);
						$("#importe_convenio").attr("disabled", true);
						$("#fecha_convenio").attr("disabled", true);
						M.toast({html: 'Todo correcto.'});
						$("#tabFin").removeClass("disabled");
						$('ul.tabs').tabs("select", "fin");
